#!/usr/bin/env python3
"""Train the final all-voice stochastic residual model with LOSO-fixed settings."""
...
LOG = logging.getLogger("train_transition_diffusion_final_v17")
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    scale = estimate_residual_scale(
        dataset, mean_model, device, args.batch_size
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in loader:
            ...
            if not torch.isfinite(loss):
                raise RuntimeError("non-finite final diffusion loss")
            loss.backward()
    ...
    checkpoint = {
        ...
        "selection_basis": (
            "fixed 10 epochs and temperatures 0.10/0.20 selected by "
            "three-fold train-only LOSO"
        ),
    ...
    report = {
        ...
        "claim_boundary": (
            "train-all artifact inherits LOSO evidence; stochastic landmark "
            "inpainting is not a human-perceptual naturalness result"
        ),
        "test_evaluated": False,
        "how2sign_validation_accessed": False,
        "how2sign_test_accessed": False,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/models/transition_residual_diffusion_v17_all6_final"),
    )
