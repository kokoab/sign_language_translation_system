#!/usr/bin/env python3
"""Train the final all-voice timing predictor with LOSO-frozen settings."""
...
LOG = logging.getLogger("train_transition_span_final_v17")
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    if len(how2sign) != 6 or len(web) < 96:
        raise ValueError("final transition timing voice breadth floor failed")
    how2sign_dataset = TransitionSpanDataset(args.how2sign_root, how2sign)
    web_dataset = TransitionSpanDataset(args.web_root, web)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in loader:
            ...
            loss = F.cross_entropy(
                model(context), target, label_smoothing=args.label_smoothing
            )
            if not torch.isfinite(loss):
                raise RuntimeError("non-finite final transition timing loss")
            loss.backward()
    ...
    checkpoint = {
        ...
        "how2sign_train_windows": len(how2sign_dataset) // 9,
        "youtube_asl_train_windows": len(web_dataset) // 9,
        ...
        "selection_basis": (
            "architecture and 40-epoch cap frozen on signer 8, confirmed on "
            "signers 3/5; fixed 39 epochs is the median selected fold epoch"
        ),
    ...
    report = {
        ...
        "claim_boundary": (
            "train-all timing artifact inherits held-out evidence and has no "
            "independent semantic timing or human-naturalness score"
        ),
        "test_evaluated": False,
        "how2sign_validation_accessed": False,
        "how2sign_test_accessed": False,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=Path("artifacts/models/transition_span_multicorpus_v17_allvoices_final"),
    )
