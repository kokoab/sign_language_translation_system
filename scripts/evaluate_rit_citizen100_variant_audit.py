#!/usr/bin/env python3
"""Use the frozen Citizen100 model to triage RIT exact-name candidates.

Model agreement is a screening signal, not proof that ASLLRP and ASL-LEX lexical
variants are identical. This script never marks a RIT clip or class training-ready.
"""
...
def aggregate_classes(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for label in sorted(grouped):
        items = grouped[label]
        clips = len(items)
        participants = len({str(item["participant"]) for item in items})
        ...
        if participants >= 2 and top1_rate >= 0.5 and top5_rate >= 0.8:
...
def main() -> None:
    ...
    parser.add_argument(
        "--feature-root",
        type=Path,
        default=Path("data/local/rit_citizen100_variant_audit/landmarks"),
    )
    parser.add_argument(
        "--provenance",
        type=Path,
        default=Path("data/local/rit_citizen100_variant_audit/candidate_provenance.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/reports/rit_citizen100_variant_audit"),
    )
    ...
    if provenance.get("training_eligible") is not False:
        raise ValueError("RIT provenance must explicitly prohibit unreviewed training use")
    metadata_by_stem = {
    ...
    for path in files:
        ...
        if key not in metadata_by_stem:
            raise ValueError(f"feature absent from RIT provenance: {path}")
        features.append(load_features(path, expected_schema))
        records.append(metadata_by_stem[key])

    if len(files) != len(metadata_by_stem):
        missing = sorted(set(metadata_by_stem) - {
            (path.parent.name, path.name.removesuffix(".v17.npz")) for path in files
        })
        raise ValueError(f"missing {len(missing)} RIT feature archives; first={missing[:1]}")
        ...
        rows.append(
            {
                ...
                "rit_entry_variant": record["rit_entry_variant"],
                "rit_occurrence": record["rit_occurrence"],
    ...
    lines = [
        ...
        "",
        f"- Candidate clips: {len(rows)}",
        f"- Candidate classes: {len(classes)}",
    ...
    for item in classes:
        lines.append(
            ...
            f"{item['citizen_asl_lex_code']} | {item['match_tier']} | "
            f"{item['clips']}/{item['participants']} | {item['top1_hits']}/{item['clips']} | "
            f"{item['top5_hits']}/{item['clips']} | {item['triage']} |"
        )
