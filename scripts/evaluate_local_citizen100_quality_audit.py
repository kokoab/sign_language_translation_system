#!/usr/bin/env python3
"""Use the frozen Citizen100 model to triage the bounded local quality shortlist.

Agreement is a mismatch screen, not accuracy evidence or lexical approval. The local
corpus has neither trustworthy signer IDs nor exact variant IDs, so this script never
marks a clip or class training-ready.
"""
...
def aggregate_classes(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for label in sorted(grouped):
        ...
        if clips >= 3 and top1_rate >= 0.5 and top5_rate >= 0.75:
        ...
        output.append(
            {
                ...
                "lexical_tier": items[0]["lexical_tier"],
                "clips": clips,
                "top1_hits": top1_hits,
                "top5_hits": top5_hits,
...
def main() -> None:
    ...
    parser.add_argument(
        "--feature-root",
        type=Path,
        default=Path("data/local/local_citizen100_quality_audit_q82/landmarks"),
    )
    parser.add_argument(
        "--provenance",
        type=Path,
        default=Path("data/local/local_citizen100_quality_audit_q82/candidate_selection.json"),
    )
    parser.add_argument(
    ...
    if provenance.get("training_eligible") is not False:
        raise ValueError("local provenance must prohibit unreviewed training use")
    metadata_by_stem = {
    ...
    for path in files:
        ...
        if key not in metadata_by_stem:
            raise ValueError(f"feature absent from local provenance: {path}")
    ...
    if len(files) != len(metadata_by_stem):
        ...
        raise ValueError(f"missing {len(missing)} local features; first={missing[:1]}")
        ...
        rows.append(
            {
                ...
                "lexical_tier": record["lexical_tier"],
    ...
    np.savez_compressed(
        ...
        targets=np.asarray([labels[str(record["canonical_label"])] for record in records], dtype=np.int64),
    ...
    summary = {
        "purpose": "model-assisted local-label triage; not an accuracy benchmark",
    ...
    lines = [
        ...
        "",
        f"- Candidate clips/classes: {len(rows)}/{len(classes)}",
    ...
    for item in classes:
        lines.append(
            ...
            f"{item['citizen_asl_lex_code']} | {item['lexical_tier']} | "
