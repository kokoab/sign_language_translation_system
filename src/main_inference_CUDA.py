...
def parse_args():
    ...
    p.add_argument('--model', '-m', type=str,
                   default=str(_SRC_DIR.parent / 'weights' / 'best_model.pth'))
    p.add_argument('--camera', '-c', type=int, default=0)
    p.add_argument('--threshold', '-t', type=float, default=0.85,
                   help='Min confidence to consider a prediction (default: 0.85)')
    p.add_argument('--stability', '-s', type=int, default=6,
                   help='Consecutive agreeing frames to accept (default: 6)')
    p.add_argument('--cooldown', type=int, default=15,
                   help='Frames to skip after accepting a sign (default: 15)')
    p.add_argument('--velocity', type=float, default=0.006,
                   help='Max hand velocity to allow classification (default: 0.006)')
    return p.parse_args()
...
def main():
    ...
    if not ckpt_path.exists():
        ...
        print('Download best_model.pth from Kaggle → weights/')
        sys.exit(1)
    ...
    while cap.isOpened():
        ...
        if res.multi_hand_landmarks and res.multi_handedness:
            ...
            if len(raw_buffer) >= 16 and is_stable and cooldown == 0:
                n_use  = min(len(raw_buffer), 40)
                raw_np = np.array(list(raw_buffer)[-n_use:], dtype=np.float32)
                ...
                if (len(stability_buf) == STAB_THRESH
                        and stability_buf[0] is not None
                        and all(x == stability_buf[0] for x in stability_buf)):
        ...
        cv2.putText(frame, f'BUF {len(raw_buffer)}/48',
