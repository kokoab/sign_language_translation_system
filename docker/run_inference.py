    ...
    if os.path.exists(STAGE1_CKPT):
        ...
        if "v14" in str(model_type):
            ...
            if ckpt1.get("ema_shadow"):
                ...
                state = ckpt1["model_state_dict"]
            s1_model.load_state_dict(state, strict=False)
            print(f"  Loaded v12 DS-GCN-TCN ({ckpt1['num_classes']} classes, d_model={ckpt1.get('d_model', 384)})")
        elif "v11" in str(model_type):
            import sys; sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))
