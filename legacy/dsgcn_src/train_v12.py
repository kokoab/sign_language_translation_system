...
from model_v12 import SLTStage1V12, count_parameters
...
def train(
    ...
    # Dataset
    cache_path = str(Path(data_path) / 'ds_cache_v12.pt')
    ...
    # Wrap training dataset to return (data, target, dataset_index)
    class IndexedSubset(Subset):
        def __getitem__(self, idx):
            x, y = super().__getitem__(idx)
            return x, y, self.indices[idx]  # also return original dataset index

    train_ds_indexed = IndexedSubset(full_ds, train_ds.indices)

    nw = min(8, os.cpu_count() or 4)
    train_loader = DataLoader(train_ds_indexed, batch_size=batch_size, sampler=sampler,
    ...
    log.info(f"Model: {count_parameters(model):,} parameters (DS-GCN-TCN)")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                # Distillation loss (per-sample when available, class-averaged fallback)
                has_distill = (per_sample_soft is not None or class_soft_labels is not None) and lam == 1.0
                if has_distill:
                    if per_sample_soft is not None and ds_indices is not None:
                        # Per-sample: each sample gets its own SAM soft label
                        teacher_probs = per_sample_soft[ds_indices].to(device)
                    elif class_soft_labels is not None:
                        # Fallback: class-averaged
...
if __name__ == "__main__":
    ...
    p.add_argument('--save_dir', default='/workspace/output_v13')
